...
| 1.1 | 2026-07-01 | 本人 | ローカル開発環境（Docker で PostgreSQL 起動）を追記（2.4） |
...
ローカルでは **Docker で PostgreSQL を起動する**。ホストへ直接 PostgreSQL をインストールせず、`docker compose` でコンテナとして立ち上げることで、環境差異をなくし、破棄・再作成を容易にする。

**方針**

- DB は Docker コンテナで起動し、NestJS アプリはホスト側（`npm run start:dev`）で動かす。アプリは `localhost:5432` へ接続する。
- 接続情報は環境変数（`DATABASE_URL`）で管理する。`.env` は **Git 管理外**（7章の方針）とし、リポジトリには記入例として `.env.example` を置く。`docker-compose.yml` は Git 管理下に置き、コンテナ側の認証情報は開発用の既定値に留める。
- データは名前付きボリュームに永続化し、コンテナを停止・削除しても消えないようにする。破棄したい場合のみ `docker compose down -v` でボリュームごと削除する。
...
```
DATABASE_URL="postgresql://postgres:postgres@localhost:5432/world_beaches?schema=public"
```
...
npx prisma migrate dev        # スキーマ適用（初回はマイグレーション作成）
npx prisma db seed            # データ投入（6章）
npm run start:dev             # アプリを起動（ホスト側）
```

- 停止：`docker compose stop`（データは保持）
- 破棄：`docker compose down -v`（ボリュームごと削除し初期状態へ）
